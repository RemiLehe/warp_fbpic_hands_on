...
# --- type of injection
top.inject      = 2    # 1=constant current, 2=space-charge limited injection
                       # type doc('inject') for more options

# --- Mesh refinement perpendicular to injection surface (off if 0)
w3d.inj_nz      = 0                       
#w3d.inj_nz      = 100                       
...
w3d.l_inj_regular = False                 #  if true, inject one particle at each grid node (random injection otherwise)
w3d.l_inj_rz = (w3d.solvergeom == w3d.RZgeom) # If using the RZ geometry, set so injection uses the same geometry
w3d.l_inj_rz_grid = (w3d.solvergeom == w3d.RZgeom)
...
if not steady_state_gun:
    ...
    pdf('Volthist_injnz%g'%w3d.inj_nz)
    print('dz = ',w3d.dz)
    print('inj_dz = ',w3d.inj_dz)
